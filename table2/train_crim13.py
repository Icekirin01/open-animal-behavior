"""
train_crim13.py — CRIM13 Video Swin-T Training (13 classes)
Video-Level Split (ensure all classes in val) + MLP Head + Cross Entropy Loss

Usage:
    python train_crim13.py --seed 123
    python train_crim13.py --seed 1337
    python train_crim13.py --seed 2025
    python train_crim13.py --seed 123 --use_class_weights
"""
...
NUM_CLASSES = 13
BEHAVIOR_NAMES = [f"category{i+1}" for i in range(NUM_CLASSES)]

DEFAULTS = dict(
    train_video_dir="data/crim13/videos/train",
    train_label_dir="data/crim13/labels",
    batch_size=8,
    ...
    seed=123,
    val_split_seed=1337,
    model_save_dir="checkpoints/crim13",
)


def parse_args():
    p = argparse.ArgumentParser(description="Train Video Swin-T on CRIM13")
...
def get_video_and_label_paths(video_dir, label_dir):
    """CRIM13 label files: {video_name}_one_hot.csv"""
    video_paths, label_paths = [], []
    for name in sorted(os.listdir(video_dir)):
        if not name.lower().endswith(".mp4"):
            continue
        vp = os.path.join(video_dir, name)
        lp = os.path.join(label_dir, name.replace(".mp4", "_one_hot.csv"))
...
def analyze_video_behaviors(video_paths, label_paths, min_threshold):
    profiles = []
...
def video_split_ensure_all_classes(video_paths, label_paths, test_size,
                                   min_threshold, random_state):
    """Random video split + move videos to ensure all classes appear in val."""
    profiles = analyze_video_behaviors(video_paths, label_paths, min_threshold)

    indices = list(range(len(profiles)))
    train_idx, val_idx = train_test_split(
        indices, test_size=test_size, random_state=random_state)

    # Ensure every class has enough val frames
    for cls in range(NUM_CLASSES):
        val_frames = sum(profiles[i]["behavior_counts"][cls] for i in val_idx)
        if val_frames < min_threshold:
            candidates = [i for i in train_idx
                          if profiles[i]["behavior_counts"][cls] >= min_threshold]
            if candidates:
                best = max(candidates, key=lambda i: profiles[i]["behavior_counts"][cls])
                train_idx.remove(best)
                val_idx.append(best)
                print(f"  Moved {profiles[best]['video_name']} to val "
                      f"(+{profiles[best]['behavior_counts'][cls]} frames of {BEHAVIOR_NAMES[cls]})")
    ...
    print(f"  {'Behavior':>15}  {'Train':>8}  {'Val':>8}  {'Status'}")
    print(f"  {'-'*45}")
    for c in range(NUM_CLASSES):
        tc = sum(profiles[i]["behavior_counts"][c] for i in train_idx)
        vc = sum(profiles[i]["behavior_counts"][c] for i in val_idx)
        status = "✅" if vc >= min_threshold else ("⚠️ LOW" if vc > 0 else "❌ MISSING")
        print(f"  {BEHAVIOR_NAMES[c]:>15}  {tc:>8}  {vc:>8}  {status}")
    print()
...
def main():
    ...
    run_tag = f"crim13_seed{args.seed}_vsplit{args.val_split_seed}"

    print(f"\n{'='*70}")
    print(f"CRIM13 Training — Swin3D + MLP Head + CE Loss")
    print(f"{'='*70}")
    ...
    # Video split
    train_vids, train_labs, val_vids, val_labs, profiles = video_split_ensure_all_classes(
    ...
    val_counts = Counter(val_ds.sample_labels)
    print(f"Train windows: {len(train_ds)}  |  Val windows: {len(val_ds)}")
    for c in range(NUM_CLASSES):
        print(f"  {BEHAVIOR_NAMES[c]:>15}: train={counts.get(c,0)}, val={val_counts.get(c,0)}")
    print()

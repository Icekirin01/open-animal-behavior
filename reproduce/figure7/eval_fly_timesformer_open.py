"""
eval_fly_timesformer_open.py — Fly Copulation TimeSformer Evaluation on Open Dataset (frame-wise)

Usage:
    python eval_fly_timesformer_open.py --model_path checkpoints/fly_timesformer_open/model.pth
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate Fly TimeSformer (frame-wise)")
    p.add_argument("--model_path", type=str, required=True)
    p.add_argument("--test_video_dir", type=str, default="data/fly_open/videos/test")
    p.add_argument("--test_label_dir", type=str, default="data/fly_open/labels/test")
...
def custom_video_transform(frames, target_size=(224,224)):
    ...
    if len(frames)!=TIMESFORMER_NUM_FRAMES: frames=uniform_sample_frames(frames,TIMESFORMER_NUM_FRAMES)
    t=[ToTensor()(f) for f in frames]; v=torch.stack(t,dim=0)
    m=torch.tensor([0.485,0.456,0.406]).view(1,-1,1,1); s=torch.tensor([0.229,0.224,0.225]).view(1,-1,1,1)
...
class WindowPredictionDataset(Dataset):
    ...
    def _gen(self):
        ...
        for vp,lp in zip(self.vps,self.lps):
            ...
            rem=filter_and_remap_labels(oh); sel=list(range(0,T,self.skip+1))
            sl_oh=np.zeros((len(sel),NUM_CLASSES))
            for i,fi in enumerate(sel): sl_oh[i,rem[fi]]=1.0
...
def main():
    ...
    print(f"\n{'='*70}")
    print("Fly Copulation — TimeSformer Open Dataset Evaluation (frame-wise)")
    print(f"{'='*70}")
    ...
    print("  Per-class F1:")
    for n,v in zip(SELECTED_BEHAVIORS,m["f1_per_class"]): print(f"    {n:>15}: {v:.4f}")
    print("\n  Per-class AP:")
    for n,v in zip(SELECTED_BEHAVIORS,m["ap_per_class"]): print(f"    {n:>15}: {v:.4f}")

    if args.save_cm:
        plot_cm(m["cm"],SELECTED_BEHAVIORS,args.save_cm,f"Fly TimeSformer Test (F1={m['f1_macro']:.4f})")
        plot_cm(m["cm"],SELECTED_BEHAVIORS,args.save_cm.replace(".png","_norm.png"),
                "Fly TimeSformer Test Normalized",normalize=True)

"""
train_fly_timesformer.py — Fly Copulation TimeSformer (ViT-Base, K400) Training
Stratified Val Split + MLP Head + CE Loss (No Data Efficiency Ratio)

Usage:
    python train_fly_timesformer.py --seed 1337
    python train_fly_timesformer.py --seed 123
    python train_fly_timesformer.py --seed 2025
"""
...
DEFAULTS = dict(
    ...
    seed=1337, val_split_seed=123,
    model_save_dir="checkpoints/fly_timesformer",
    hf_model="facebook/timesformer-base-finetuned-k400",
)


def parse_args():
    p = argparse.ArgumentParser(description="Train TimeSformer on Fly Copulation")
...
def main():
    ...
    run_tag = f"fly_timesformer_seed{args.seed}_vsplit{args.val_split_seed}"

    print(f"\n{'='*70}")
    print(f"Fly Copulation — TimeSformer (ViT-Base, K400) + MLP Head")
    print(f"{'='*70}")

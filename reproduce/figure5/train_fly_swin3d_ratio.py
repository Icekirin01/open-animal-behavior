"""
train_fly_swin3d_ratio.py — Fly Copulation Video Swin-T Training
Stratified Val Split + Video-Level Data Efficiency + MLP Head + CE Loss

Best model selected by val F1-macro excluding 'others'.

Usage:
    python train_fly_swin3d_ratio.py --train_data_ratio 0.75 --video_split_seed 42
    python train_fly_swin3d_ratio.py --train_data_ratio 0.5 --video_split_seed 123
    python train_fly_swin3d_ratio.py --train_data_ratio 1.0
"""
...
DEFAULTS = dict(
    ...
    video_split_seed=42,
    model_save_dir="checkpoints/fly_swin3d_ratio",
)


def parse_args():
    p = argparse.ArgumentParser(description="Train Video Swin-T on Fly Copulation (ratio)")
    ...
    p.add_argument("--train_data_ratio", type=float, default=DEFAULTS["train_data_ratio"],
                   help="Fraction of training videos to use (1.0=all)")
    p.add_argument("--video_split_seed", type=int, default=DEFAULTS["video_split_seed"],
                   help="RNG seed for video-level subsampling")
...
def analyze_and_split(video_paths, label_paths, test_size, min_thresh, random_state):
    ...
    try:
        if use_strat:
            tr, va = train_test_split(indices, test_size=test_size, random_state=random_state, stratify=strat)
        else:
            tr, va = train_test_split(indices, test_size=test_size, random_state=random_state)
    except ValueError:
        tr, va = train_test_split(indices, test_size=test_size, random_state=random_state)
...
def subsample_videos_by_ratio(vps, lps, ratio, rng_seed=42):
    if ratio >= 1.0:
        return vps, lps
    n = len(vps)
...
def count_frames_per_behavior(vps, lps):
...
def evaluate_window_level(model, loader, device):
    ...
    f1_no_others = float(np.mean(f1_pc[non_others]))
    ...
    return {"f1_per_class": f1_pc, "f1_macro": f1_m, "f1_no_others": f1_no_others,
...
def main():
    ...
    run_tag = f"fly_swin3d_{ratio_tag}{split_tag}_seed{args.seed}"

    print(f"\n{'='*70}")
    print(f"Fly Copulation — Swin3D + MLP Head (Data Efficiency)")
    print(f"{'='*70}")
    ...
    frame_counts, total_frames = count_frames_per_behavior(train_v, train_l)
    total_win = sum(counts.values())

    print(f"{'='*60}")
    print(f"📊 Data Report (ratio={args.train_data_ratio}, vseed={args.video_split_seed})")
    print(f"{'='*60}")
    print(f"  Train frames: {total_frames} | Train windows: {len(train_ds)} | Val windows: {len(val_ds)}")
    print(f"\n  {'Behavior':>15}  {'Frames':>8}  {'Windows':>8}  {'Frame%':>7}  {'Win%':>7}")
    print(f"  {'-'*53}")
    for c in range(NUM_CLASSES):
        fc, wc = frame_counts.get(c, 0), counts.get(c, 0)
        print(f"  {SELECTED_BEHAVIORS[c]:>15}  {fc:>8}  {wc:>8}  "
              f"{100*fc/total_frames if total_frames else 0:>6.2f}%  "
              f"{100*wc/total_win if total_win else 0:>6.2f}%")
    print(f"{'='*60}\n")
    ...
    with open(log_path, "w") as f:
        json.dump({
            ...
            "split_info": {
                "train_pool_videos": [os.path.basename(v) for v in pool_v],
                "train_videos_used": [os.path.basename(v) for v in train_v],

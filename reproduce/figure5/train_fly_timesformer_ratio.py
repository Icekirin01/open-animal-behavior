"""
train_fly_timesformer_ratio.py — Fly Copulation TimeSformer (ViT-Base, K400) Training
Stratified Val Split + Video-Level Data Efficiency + MLP Head + CE Loss

Best model selected by val F1-macro excluding 'others'.

Usage:
    python train_fly_timesformer_ratio.py --train_data_ratio 0.75 --video_split_seed 42
    python train_fly_timesformer_ratio.py --train_data_ratio 0.5 --video_split_seed 123
    python train_fly_timesformer_ratio.py --train_data_ratio 1.0
"""
...
DEFAULTS = dict(
    ...
    seed=2025, val_split_seed=123, train_data_ratio=1.0, video_split_seed=42,
    model_save_dir="checkpoints/fly_timesformer_ratio",
    hf_model="facebook/timesformer-base-finetuned-k400",
)


def parse_args():
    p = argparse.ArgumentParser(description="Train TimeSformer on Fly Copulation (ratio)")
...
def analyze_and_split(vps, lps, test_size, min_thresh, random_state):
    ...
    for vp, lp in zip(vps, lps):
        ...
        except Exception as e: print(f"⚠️ {vp}: {e}")
...
def count_frames_per_behavior(vps, lps):
    counts={c:0 for c in range(NUM_CLASSES)}; total=0
    for vp,lp in zip(vps,lps):
        for l in filter_and_remap_labels(pd.read_csv(lp).iloc[:,0:len(ALL_BEHAVIOR_NAMES)].values):
            counts[l]+=1; total+=1
    return counts, total
...
def evaluate_window_level(model, loader, device):
    ...
    return {"f1_per_class":f1_pc,"f1_macro":f1_m,"f1_no_others":f1_no,"ap_per_class":ap_pc,"mAP":np.mean(ap_pc)}
...
def main():
    ...
    run_tag=f"fly_timesformer_{ratio_tag}{split_tag}_seed{args.seed}"

    print(f"\n{'='*70}")
    print(f"Fly Copulation — TimeSformer (ViT-Base, K400) + MLP Head (Data Efficiency)")
    print(f"{'='*70}")
    ...
    if args.train_data_ratio<1.0: print(f"  Selected: {[os.path.basename(v) for v in train_v]}")
    ...
    fc,tf=count_frames_per_behavior(train_v,train_l); tw=sum(counts.values())
    ...
    for epoch in range(args.num_epochs):
        ...
        log.append({"epoch":epoch+1,"train_loss":el,"val_f1":m["f1_macro"],"val_f1_no_others":m["f1_no_others"],
                     "val_map":m["mAP"],"val_f1_per_class":m["f1_per_class"].tolist(),
    ...
    with open(lp,"w") as f:
        json.dump({"training_log":log,"best_epoch":best_ep,"best_model_path":best_path,
                    "config":{**vars(args),"num_classes":NUM_CLASSES,"selected_behaviors":SELECTED_BEHAVIORS,
                              ...
                              "model":"TimeSformer (ViT-Base, K400) + MLP Head"},
